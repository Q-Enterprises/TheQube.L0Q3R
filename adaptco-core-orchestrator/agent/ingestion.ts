import Ajv from "ajv";
import plugSchema from "./cie_v2_plug.v1.schema.json";
import invariantSchema from "./invariant_set.v1.schema.json";
...
export function ingestPlug(raw: unknown): AgentContext {
  if (!validatePlug(raw)) {
    throw new Error(
      "cie_v2_plug.v1 validation failed: " +
        ajv.errorsText(validatePlug.errors, { separator: "\n" })
    );
  }
