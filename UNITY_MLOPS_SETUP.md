...
| `VERTEX_SERVING_CONTAINER_IMAGE_URI` | No* | Vertex prediction container image (ONNX Runtime image required for `.onnx` artifacts) |
| `VERTEX_SERVING_CONTAINER_PREDICT_ROUTE` | No* | Predict endpoint route exposed by the serving container (`/predict` default) |
| `VERTEX_SERVING_CONTAINER_HEALTH_ROUTE` | No* | Health endpoint route exposed by the serving container (`/health` default) |
